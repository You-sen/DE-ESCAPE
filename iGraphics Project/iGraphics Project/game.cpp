...
int mcover;
int cover,start,story=0,about,control,exitt,back,next,solvers;
int yousuf,togar,abir,rasel=0;
int stoscr=0,conscr=0,aboscr=0,solvscr=0;
int bg[20]={};
int dl[54];
int i=0,j=0;
int screen=0,carsc=0;
int cha,car,cx=950,cy=150,cax=1200,cay=0,head=0,headx=924,heady=320;
...
int rid,rid1,rid2=0,rid3,rid4,rid5,rid6;
int sto1=0,stopow1=0,sto2=0,stopow2=0;
...
/**/char str[100], str2[100],str3[100],s[]={'s','e','e','d','\0'};
int len;
int mode;
int flag=0,flag2=0;
int winnerCounter=0;
...
void bg14(int butt, int sta, int max, int may)
{
	if(max >= 924 && max <= 1024 && may >= 0 && may <= 50)
			{
				while(j<55){
...
void bg15(int butt, int sta, int max, int may)
{
	if(max >= 924 && max <= 1024 && may >= 0 && may <= 50)
			{
				while(j<56){
...
void bg16(int butt, int sta, int max, int may)
{
	if(max >= 924 && max <= 1024 && may >= 0 && may <= 50)
			{
				while(j<57){
...
void bg17(int butt, int sta, int max, int may)
{
	if(max >= 924 && max <= 1024 && may >= 0 && may <= 50)
			{
				while(j<58){
...
void synch(){
    ...
    if(cx<=1200&&cx>800) j=54; 
	}
	else if(i==15){
    if(cx<=1200&&cx>800) j=55; 
	}
	else if(i==16){
    if(cx<=1200&&cx>800) j=56; 
	}
	else if(i==17){
    if(cx<=1200&&cx>300) j=57; 
	}
...
if(screen==0){
	...
	iShowImage(924,590, 100, 50, solvers);
	...
	iShowImage(0, 195, 200, 200, yousuf);
	iShowImage(310, 195, 200, 200, togar);
	iShowImage(580, 195, 200, 200, abir);
	iShowImage(824, 195, 200, 200, rasel);

	iShowImage(924, 0, 100, 50, back);
	}
else if(screen==5)
	{
	iShowImage(0, 0, 1024, 640, solvscr);
	int namex=10,namey=500;
	for(int nl=0;nl<=winnerCounter;nl++)
	{
	  iSetColor(255, 255, 255);
	  iText(namex, namey,winner[nl].name,GLUT_BITMAP_TIMES_ROMAN_24);
	  if(namey>=30)
	   namey=namey-30;
	  else
	  {
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		if(screen==0){
			if(mx >= 0 && mx <= 100 && my >= 590 && my <= 640)
				...
				screen=4;
			else if(mx >= 924 && mx <= 1024 && my >= 590 && my <= 640)
				screen=5;
		}
		else if(screen==2){
			...
			if(i==14)
				bg14(button,state,mx,my);
			if(i==15)
				bg15(button,state,mx,my);
			if(i==16)
				bg16(button,state,mx,my);
			if(i==17)
				bg17(button,state,mx,my);
...
void iKeyboard(unsigned char key)
{
	...
	if(mode == 1)
	{
	    if(i==17){
			...
			strcpy(str3, str);
			printf("%s\n", str3);
			p=fopen("winner.txt","a");
			fprintf(p,"%s\n",str3);
			strcpy(winner[winnerCounter].name,str3);
			winnerCounter++;
			fclose(p);
			screen=0;
			i=0;
...
void iSpecialKeyboard(unsigned char key)
{
	...
	if (key == GLUT_KEY_UP)
	{
		if(i!=8&&i!=9&&i!=10&&i!=12&&i!=18&&i!=19){	
			if(i==0||i>2){
				if(i==14){
				carsc=3;
				if(heady>=590)
...
void file(){
	int i=0;
p=fopen("winner.txt","r");
if(p==NULL){
}
while(fscanf(p,"%s",&winner[i].name)!=EOF){

	printf("%s",winner[i].name);
	i++;
	winnerCounter++;
}
fclose(p);
...
int main()
{
	...
	*/
	...
	next = iLoadImage("BT\\next.png");
	solvers = iLoadImage("BT\\solvers.png");
	...
	solvscr = iLoadImage("BT\\solscr.png");
	yousuf = iLoadImage("BT\\yousuf.png");
	togar = iLoadImage("BT\\togar.png");
	abir = iLoadImage("BT\\abir.png");
	rasel = iLoadImage("BT\\rasel.png");
	...
	bg[16] = iLoadImage("BG\\endpass.png");
	bg[17] = iLoadImage("BG\\namebg.png");
	bg[18] = iLoadImage("BG\\go3.png");
	bg[19] = iLoadImage("BG\\go4.png");
	...
	dl[55] = iLoadImage("DL\\56.png");
	dl[56] = iLoadImage("DL\\57.png");
	dl[57] = iLoadImage("DL\\58.png");
	//dl[58] = iLoadImage("DL\\55.png");
